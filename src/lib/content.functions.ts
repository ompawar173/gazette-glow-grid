...
export const getArticlePage = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    try {
      ...
      const db = publicDb();
      const cleanSlug = decodeURIComponent(data.slug).trim();

      let { data: article } = await db
        .from("articles")
        .select("*")
        .eq("slug", cleanSlug)
        .eq("status", "published")
      ...
      if (!article) return { article: null, related: [], categorySlug: null, subSlug: null };
      ...
      console.error("[getArticlePage] Error:", e);
      return { article: null, related: [], categorySlug: null, subSlug: null };
    }
  });
